	...
	public class ListIndexKeys
	{
		...
		public void ListKeys( string sIndexName, string DatabaseName)
		{
			try
			{
				...
				sSQL  = "select name, colstat from sysindexkeys inner join syscolumns on sysindexkeys.colid = syscolumns.colid where ";
				...
				sSQL += "and syscolumns.id=(select id from sysindexes where name='" + sIndexName + "')";
